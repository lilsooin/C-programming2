...
namespace codesample.array_of_arrays
{
    class ArrayOfArrays
    {
        public static void ArrayOfArraysRun()
        {
            const int MONTHS_IN_A_YEAR = 12;
            int[] daysInEchMonth = new int[MONTHS_IN_A_YEAR] { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

            string[][] calendar = new string[MONTHS_IN_A_YEAR][];

            for (int i = 0; i < MONTHS_IN_A_YEAR; ++i)
            {
                calendar[i] = new string[daysInEchMonth[i]];
            }
            ...
            while (true)
            {
                ...
                Console.WriteLine($"Enter the Day (1 - {calendar[month - 1].Length}): ");
